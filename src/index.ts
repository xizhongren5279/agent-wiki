...
server.tool(
  "wiki_status",
  "查询知识库状态。首次调用如果返回 initialized: false，你应该主动引导用户：1. 问知识库目录 2. 问兴趣方向 3. 调用 wiki_init。不要等用户说'帮我建知识库'，你主动开始。",
  {},
  async () => handleStatus()
);
...
server.tool(
  "wiki_init",
  "初始化知识库。创建目录结构和 SCHEMA.md。需要用户提供：目录路径、关注主题、兴趣方向。",
  {
...
server.tool(
  "wiki_ingest",
  "摄入文件到知识库。支持 md/txt/html/csv（自动转 markdown）、pdf/docx/xlsx/图片（原样存储）。从指定路径复制到 raw/ 目录（增量，已摄入的跳过）。返回新文件内容预览，供你用 LLM 提取标签和摘要。",
  {
...
server.tool(
  "wiki_tag",
  "将标签写入文件的 frontmatter。你应该先用 LLM 为文件提取 5-10 个标签，然后调用此工具写入。",
  {
...
server.tool(
  "wiki_compile",
  "根据你用 LLM 生成的摘要、洞察、分类生成 Wiki 页面。同时更新 INDEX.md 和 LOG.md。你应该先读 SCHEMA.md 了解 Wiki 页面格式，用 LLM 生成内容后再调用。",
  {
...
server.tool(
  "wiki_search",
  "搜索知识库。支持按标签和关键词搜索，返回匹配的 Wiki 页面列表。标签匹配权重 0.6 + 全文匹配权重 0.4。",
  {
...
server.tool(
  "wiki_article",
  "保存原创文章到 outputs/ 目录。保存前，你必须先读 SCHEMA.md 的'写原创文章规则'，确保文章是有人味儿的微信公众号风格，没有 AI 味。",
  {
...
server.tool(
  "wiki_lint",
  "自检知识库健康状态。检测死链接、缺失摘要、内容过短、缺少标签等问题，返回问题列表。",
  {
...
server.tool(
  "wiki_feedback",
  "保存问答分析到知识库。仅当用户明确说'存'或'保存'时才调用。将问答中的综合分析、对比、洞察保存为新的 Wiki 页面，标记为 query-derived 类型（区分原始材料编译 vs 问答生成）。",
  {
