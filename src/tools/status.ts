...
import { getConfig, getWikiDir, getRawDir, getWikiPagesDir, getOutputsDir } from "../state.js";
...
export async function handleStatus(): Promise<ToolResult> {
  ...
  const wikiFiles = listMdFiles(wikiPagesDir).filter(
    (f) => f !== "INDEX.md" && f !== "LOG.md"
  );
  ...
  for (const f of wikiFiles) {
    ...
    if (Array.isArray(meta.tags)) {
      for (const t of meta.tags) allTags.add(String(t));
    }
  }
